...
1. Creamos una variable global al iniciodel código para guardar nuestrosonido.  
2. Usamos la función functionpreload() inicializamos nuestravariable y cargamos el sonido conloadSound()
...
* nombreVariable.**loop()**: Detiene el sonido por completo.  
...
* nombreVariable.**isPlaying()**: Devuelve true si el sonido está.  
...
## DESAFÍO DE LA CLASE: RADIO CON PLAY/ PAUSE
...
function preload() {
  miSonido = loadSound("SONIDO/espada.mp3");
  miImagen = loadImage("Imagen/miRadio.png");
}

function setup() {
  createCanvas(600, 500);
    
  botonPlay = createButton('Play'); // Creamos el botón y lo nombramos PLAY
  botonPlay.position(225, 430); // Posición en la pantalla
  botonPlay.mousePressed(miSonido);// Cuando se haga clic en ÉSTE botón, se ejecuta el sonido

  botonPause = createButton('Pause'); // Creamos el botón y lo nombramos PAUSE
  botonPause.position(450, 430); // Posición en la pantalla
  botonPause.mousePressed(miSonido); // Cuando se haga clic en ÉSTE botón, se detiene el sonido
}

function draw() {
  background(255, 210, 201);
    imageMode(CENTER);
    image(miImagen, width / 2, height / 2, width, height);
}

function mousePressed() {
  if (miSonido.isPlaying()) {
    miSonido.stop(); // Si ya está sonando, lo detiene
  } else {
    miSonido.play(); // Si está detenido, le da play
  }
}
```
---
...
![ejemplo](https://github.com/user-attachments/assets/792ecc70-8e6f-4ea1-a611-d261d3e80056)
